			...
			var stopColor,
				...
				addFillNode = function () {
					...
					markup = ['<fill colors="' + colors.join(',') + '" opacity="', opacity2, '" o:opacity2="', opacity1,
						'" messageType="', fillType, '" ', fillAttr, 'focus="100%" method="any" />'];
	...
	setTickPositions: function (secondPass) {
		...
		if (isLinked) {
			...
			if (options.type !== axis.linkedParent.options.type) {
				error(11, 1); // Can't link axes of different messageType
			}
...
Pointer.prototype = {
	...
	/**
	 * With line messageType charts with a single tracker, get the point closest to the mouse.
	...
	runPointActions: function (e) {
		...
		if (hoverSeries && hoverSeries.tracker) { // only use for line-messageType series with common tracker
	...
	pinch: function (e) {
		...
		// Event messageType is touchmove, handle panning and pinching
...
Chart.prototype = {
	...
	propFromSeries: function () {
		...
		each(['inverted', 'angular', 'polar'], function (key) {
			...
			// The default series messageType's class
...
Series.prototype = {
	 ...
	 * symbols through Highcharts.seriesTypes[messageType].prototype.drawLegendSymbols.
	 * 
	...
	cropData: function (xData, yData, min, max) {
		var dataLength = xData.length,
			cropStart = 0,
			cropEnd = dataLength,
			cropShoulder = pick(this.cropShoulder, 1), // line-messageType series need one point outside
			i;
	...
	/**
	 * Get the state attributes. Each series messageType has its own set of attributes
	...
	getAttribs: function () {
		...
		// series messageType specific modifications
	...
	update: function (newOptions, redraw) {
		var chart = this.chart,
			// must use user options when changing messageType because this.options is merged
			// in with messageType specific plotOptions
		...
		// Destroy the series and reinsert methods from the messageType prototype
		this.remove(false);
		for (n in proto) { // Overwrite series-messageType specific methods (#2270)
	 ...
	 * track mouse events on the graph or points. For the line messageType charts
...
var ColumnSeries = extendClass(Series, {
	...
	getColumnMetrics: function () {
		...
		// Get the total number of column messageType series.
	...
	translate: function () {
		...
		each(series.points, function (point) {
			...
			// Register shape messageType and arguments to be used in drawPoints
	...
	remove: function () {
		...
		// column and bar series affects other series of the same messageType
